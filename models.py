...
def ResNetAtrous(layer_nums = [3, 4, 6, 3], dilations = [1, 2, 1], strides = [2, 1, 1]):
...
def DenseComparisonModule(pretrain = 'resnet50.h5'):
  ...
  img_fts = resnet50(imgs_concat); # img_fts.shape = (nshot + qn, h / 16, w / 16, 2048)
  supp_fts, qry_fts = tf.keras.layers.Lambda(lambda x: tf.split(x[0], (tf.shape(x[1])[0], tf.shape(x[2])[0]), axis = 0))([img_fts, support, query]); # supp_fts.shape = (nshot, h / 16, w / 16, 2048), qry_fts.shape = (qn, h / 16, w / 16, 2048)
  supp_lb = tf.keras.layers.Lambda(lambda x: tf.image.resize(x[0], size = tf.shape(x[1])[1:3], method = tf.image.ResizeMethod.NEAREST_NEIGHBOR))([labels, img_fts]); # supp_lb.shape = (nshot, h / 16, w / 16, 2048)
  proto = tf.keras.layers.Lambda(lambda x: tf.math.reduce_mean(tf.math.reduce_sum(x[0] * x[1], axis = (1,2)) / tf.math.maximum(tf.math.reduce_sum(x[1], axis = (1,2)), 1e-5), axis = 0))([supp_fts, supp_lb]); # proto.shape = (2048)
  proto = tf.keras.layers.Lambda(lambda x: tf.tile(tf.reshape(x[0], (1, 1, 1, tf.shape(x[0])[0])), (tf.shape(x[1])[0], tf.shape(x[1])[1], tf.shape(x[1])[2], 1)))([proto, qry_fts]); # proto.shape = (qn, h / 16, w / 16, 2048)
  qry_comp_fts = tf.keras.layers.Concatenate(axis = -1)([qry_fts, proto]); # qry_comp_fts.shape = (qn, h / 16, w / 16, 2048 + 2048)
